...
Item {
  ...
  function runUpdate(kind, agentIds) {
    ...
    if (ollamaEnabled() && ollamaCollector !== "") {
      ...
      updateProcess.command = base
    }
    updateProcess.running = true
  }

  // ------------------------------------------------- built-in ollama collector

  // collect.py renders ollama.com/settings through the desktop browser's
  // session and writes ollama.json into the usage directory this component
  // already watches. Qt.resolvedUrl().toString() strips the file:// scheme so
  // python sees a plain path; a third-party manifest's __sourceDir is not
  // exposed, so the module URL is the only reliable way to find the script.
  readonly property string ollamaCollector: {
    var url = Qt.resolvedUrl("collect.py").toString()
    return url.indexOf("file://") === 0 ? url.substring(7) : url
  }

  function ollamaEnabled() {
    return settings && settings.providers && settings.providers.ollama
  ...
  function displayProvider(record) {
    ...
    return {
      ...
      modelRequests: record.modelRequests || {},
