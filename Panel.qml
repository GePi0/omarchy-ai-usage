...
Panel {
  id: root
  moduleName: "ollama-cloud.ai-usage"
  ipcTarget: "ollama-cloud.ai-usage"
  manageIpc: false
  ...
  readonly property bool refreshingProvider: refreshing || usage.updateRunning
  ...
  readonly property var requestList: requestRows(provider)
  ...
  function modelTooltip(row) {
    if (!row) return ""
    if (row.kind === "requests") return Number(row.total) + " requests this week"
  ...
  // Ollama Cloud reports per-model request counts instead of token buckets.
  function requestRows(p) {
    var requestsByModel = p ? (p.modelRequests || {}) : {}
    var rows = []
    for (var id in requestsByModel) {
      var count = Number(requestsByModel[id] || 0)
      if (count > 0) rows.push({ name: usage.friendlyModelName(id), total: count, kind: "requests" })
    }
    rows.sort(function(a, b) { return b.total - a.total })
    return rows.slice(0, 4)
  }
  ...
  Main {
    ...
    onRefreshFinished: root.refreshing = false
  }
        ...
        Column {
          ...
          PanelHero {
            ...
            trailingControl: Component {
              PanelActionButton {
                iconText: root.refreshingProvider ? "󰤄" : "󰑐"
                tooltipText: root.refreshingProvider ? "Refreshing…" : "Refresh (r)"
                foreground: root.foreground
                hasCursor: true
                enabled: !root.refreshingProvider
          ...
          // ---------- Models ----------
          PanelSeparator {
            visible: requestSection.visible || modelSection.visible
            foreground: root.foreground
          }

          Column {
            id: requestSection
            visible: root.requestList.length > 0
            width: parent.width
            spacing: Style.spacing.md

            PanelSectionHeader {
              width: parent.width
              text: "MODELS USED THIS WEEK"
              foreground: root.foreground
              fontFamily: root.fontFamily
            }

            Repeater {
              model: root.requestList

              ModelRow {
                required property var modelData
                width: requestSection.width
                row: modelData
                share: modelData.total / Math.max(1, root.requestList[0].total)
              }
            }
          }
    ...
    Item {
      ...
      Text {
        ...
        text: limitRow.window && limitRow.window.percent >= 0
          ? (Math.round(limitRow.window.percent * 1000) / 10) + "%"
          : "—"
  ...
  component ModelRow: Item {
    ...
    Text {
      ...
      textFormat: Text.PlainText
      text: {
        if (!modelRow.row) return ""
        if (modelRow.row.kind === "requests")
          return modelRow.row.total + (modelRow.row.total === 1 ? " request" : " requests")
        return usage.formatTokenCount(modelRow.row.total)
      }
      color: root.dim
      font.family: root.fontFamily
      font.pixelSize: Style.font.bodySmall
